...
import { countRenderableVoxelStates, RENDERABLE_STATES } from '../renderingData';
...
export class OrganicRenderer {
  ...
  private instancedMeshCapacities: Map<VoxelState, number> = new Map();
  private readonly stateVoxelCounts = new Uint32Array(VoxelState.Corrupted + 1);
  ...
  private renderSpheres(grid: VoxelGrid): void {
    ...
    const stateCounts = countRenderableVoxelStates(data, this.stateVoxelCounts);

    for (const state of RENDERABLE_STATES) {
      this.ensureInstancedMesh(state, stateCounts[state] ?? 0);
    }
    ...
    const indices = new Uint32Array(VoxelState.Corrupted + 1);
    ...
    for (let z = 0; z < depth; z++) {
      ...
      for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x++) {
          ...
          const instancedMesh = this.instancedMeshes.get(state);
          if (!instancedMesh) {
            continue;
          }

          const index = indices[state] ?? 0;
          indices[state] = index + 1;
          matrix.setPosition(
    ...
    for (const state of RENDERABLE_STATES) {
      const instancedMesh = this.instancedMeshes.get(state);
      const visibleCount = stateCounts[state] ?? 0;
      if (!instancedMesh) {
        continue;
      }

      instancedMesh.count = visibleCount;
      instancedMesh.instanceMatrix.needsUpdate = visibleCount > 0;
      instancedMesh.visible = visibleCount > 0;
    }
  }

  private ensureInstancedMesh(state: VoxelState, requiredCapacity: number): void {
    let instancedMesh = this.instancedMeshes.get(state);
    const currentCapacity = this.instancedMeshCapacities.get(state) ?? 0;
    ...
    if (instancedMesh && currentCapacity >= requiredCapacity) {
      return;
    }
    ...
    const capacity = Math.max(Math.ceil(requiredCapacity * 1.5), 100);
    ...
    this.instancedMeshCapacities.set(state, capacity);
    this.scene.add(instancedMesh);
  }

  private hideInstancedMeshes(): void {
  ...
  public dispose(): void {
    ...
    this.instancedMeshCapacities.clear();
