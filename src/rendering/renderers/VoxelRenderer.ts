...
import { countRenderableVoxelStates, fillPointCloudBuffers, getRenderableVoxelCount } from '../renderingData';
...
export class VoxelRenderer {
  ...
  private pointsPositionAttribute: THREE.BufferAttribute | null = null;
  private pointsColorAttribute: THREE.BufferAttribute | null = null;
  private pointsCapacity: number = 0;
  private readonly pointStateCounts = new Uint32Array(VoxelState.Corrupted + 1);
  ...
  public setLightweightMode(enabled: boolean): void {
    if (this.useLightweightMode === enabled) {
      return;
    }
  ...
  private renderGridLightweight(grid: VoxelGrid): void {
    ...
    const stateCounts = countRenderableVoxelStates(data, this.pointStateCounts);
    const voxelCount = getRenderableVoxelCount(stateCounts);

    this.ensurePointsCapacity(voxelCount);

    const index = fillPointCloudBuffers(
      grid,
      this.voxelSize,
      CACHED_COLORS,
    ...
    this.pointsPositionAttribute!.needsUpdate = true;
    this.pointsColorAttribute!.needsUpdate = true;
    this.pointsGeometry!.setDrawRange(0, index);
  }

  private ensurePointsCapacity(voxelCount: number): void {
    const minimumCapacity = Math.max(voxelCount, 1000);
    ...
    if (this.pointsCapacity >= minimumCapacity && this.pointsPositions && this.pointsColors) {
      if (this.pointsMesh && !this.scene.children.includes(this.pointsMesh)) {
        this.scene.add(this.pointsMesh);
      }
      return;
    }

    this.pointsCapacity = minimumCapacity;
    this.pointsPositions = new Float32Array(this.pointsCapacity * 3);
    this.pointsColors = new Float32Array(this.pointsCapacity * 3);

    this.pointsPositionAttribute = new THREE.BufferAttribute(this.pointsPositions, 3);
    this.pointsPositionAttribute.setUsage(THREE.DynamicDrawUsage);
    this.pointsColorAttribute = new THREE.BufferAttribute(this.pointsColors, 3);
    this.pointsColorAttribute.setUsage(THREE.DynamicDrawUsage);

    this.pointsGeometry.setAttribute('position', this.pointsPositionAttribute);
    this.pointsGeometry.setAttribute('color', this.pointsColorAttribute);
  }

  private disposePointsMesh(): void {
    if (this.pointsMesh) {
      this.scene.remove(this.pointsMesh);

      if (Array.isArray(this.pointsMesh.material)) {
        this.pointsMesh.material.forEach(material => material.dispose());
      } else {
    ...
    this.pointsPositionAttribute = null;
    this.pointsColorAttribute = null;
    this.pointsPositions = null;
    this.pointsColors = null;
    this.pointsCapacity = 0;
  }
